...
public class FloorCeiling implements Border {
    ...
    public boolean isOn(final Point location) {
        return getArea().isVisible() && (getY() == location.y) && (getLeft() <= location.x) && (location.x <= getRight());
    }
    ...
    public Point move(final Point location) {
        ...
        final Point newLocation = new Point((location.x - (getLeft() - getDLeft())) * (getRight() - getLeft()) / d + getLeft(), location.y + getDY());
