...
public final class Sound {
    ...
    Sound(@NotNull AudioInputStream paramAudioInputStream) throws IOException {
        ...
        int i = localAudioFormat.getChannels();
        int j = localAudioFormat.getSampleSizeInBits();
        ...
        final ByteBuffer localByteBuffer = ByteBuffer.allocateDirect(size);
        while (localByteBuffer.remaining() > 0) {
            localReadableByteChannel.read(localByteBuffer);
        }
        localByteBuffer.rewind();

        if ((j == 16) && (ByteOrder.nativeOrder() == ByteOrder.BIG_ENDIAN)) {
