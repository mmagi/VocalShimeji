...
public class Move extends BorderedAction {
    ...
    protected void tick() throws LostGroundException, VariableException {
        ...
        if (targetX != DEFAULT_TARGETX) {
            // 行き過ぎた分戻す
            if ((getMascot().isLookRight() && (getMascot().getAnchor().x >= targetX)) || (!getMascot().isLookRight() && (getMascot().getAnchor().x <= targetX))) {
                getMascot().setAnchor(new Point(targetX, getMascot().getAnchor().y));
            }
        }
        if (targetY != DEFAULT_TARGETY) {
            // 行き過ぎた分戻す
            if ((down && (getMascot().getAnchor().y >= targetY)) || (!down && (getMascot().getAnchor().y <= targetY))) {
